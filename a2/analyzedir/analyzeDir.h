...
struct Results
{
  ...
  long all_files_size;
  // largest directories (only including immediate files and not sub-folders, don't include empty directories with no files)
  std::vector<std::pair<std::string, int>> largest_dirs;
  // most common words found in .txt files
  // word = sequence longer than 5 alphabetic characters, converted to lower case
