  ...
  try {
    ...
    const uaCounts = uaData?.reduce((acc: Record<string, number>, visit) => {
      if (visit.user_agent) {
        ...
        const simplified = visit.user_agent.includes('Chrome') ? 'Chrome' :
                         visit.user_agent.includes('Firefox') ? 'Firefox' :
                         visit.user_agent.includes('Safari') ? 'Safari' :
                         visit.user_agent.includes('Edge') ? 'Edge' :
                         'Other';
    ...
    // Get raw visits (last 100)
    const { data: rawData, error: rawError } = await supabase
      .from('visitor_analytics')
      .select('id, timestamp, ip_address, page_path, user_agent, country, device_type')
